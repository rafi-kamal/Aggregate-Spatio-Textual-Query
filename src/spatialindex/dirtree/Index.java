...
public class Index extends Node
{
	...
	protected Node chooseSubtree(Region mbr, int level, Stack pathBuffer, HashSet doc)
	{
	...
	protected Node[] split(byte[] pData, Region mbr, int id, HashSet doc)
	{
		...
		switch (m_pTree.m_treeVariant)
		{
			case SpatialIndex.RtreeVariantLinear:
			case SpatialIndex.RtreeVariantQuadratic:
				rtreeSplit(pData, mbr, id, g1, g2, doc);
				break;
			case SpatialIndex.RtreeVariantRstar:
		...
		for (cIndex = 0; cIndex < g1.size(); cIndex++)
		{
			...
			left.insertEntry(null, m_pMBR[i], m_pIdentifier[i], m_pDoc[i]);
		}

		for (cIndex = 0; cIndex < g2.size(); cIndex++)
		{
			int i = ((Integer) g2.get(cIndex)).intValue();
			right.insertEntry(null, m_pMBR[i], m_pIdentifier[i], m_pDoc[i]);
		}
	...
	protected int findLeastEnlargement(Region r, HashSet doc)
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			double score = DIRTree.beta_area*enl + (1-DIRTree.beta_area)*docdistance(doc, m_pDoc[cChild]);
	...
	protected void adjustTree(Node n, Stack pathBuffer)
	{
		...
		m_pDoc[child] = (HashSet)n.m_doc.clone();
	...
	protected void adjustTree(Node n1, Node n2, Stack pathBuffer, boolean[] overflowTable)
	{
		...
		m_pDoc[child] = (HashSet)n1.m_doc.clone();
		...
		boolean adjusted = insertData(null, (Region) n2.m_nodeMBR.clone(), n2.m_identifier, pathBuffer, overflowTable, (HashSet)n2.m_doc.clone());
