...
abstract class Node implements INode
{
	...
	protected HashSet[] m_pDoc = null;
	protected HashSet m_doc = null;
	...
	protected abstract Node chooseSubtree(Region mbr, int level, Stack pathBuffer, HashSet doc);
	protected abstract Leaf findLeaf(Region mbr, int id, Stack pathBuffer);
	protected abstract Node[] split(byte[] pData, Region mbr, int id, HashSet doc);
	...
	protected Node(DIRTree pTree, int id, int level, int capacity)
	{
		...
		m_pDoc = new HashSet[m_capacity + 1];
		m_doc = new HashSet();
	}

	protected void insertEntry(byte[] pData, Region mbr, int id, HashSet doc) throws IllegalStateException
	{
	...
	protected boolean insertData(byte[] pData, Region mbr, int id, Stack pathBuffer, boolean[] overflowTable, HashSet doc)
	{
		if (m_children < m_capacity)
		{
			boolean adjusted = false;
			boolean b = m_nodeMBR.contains(mbr);
			boolean flag = m_doc.containsAll(doc);

			insertEntry(pData, mbr, id, doc);
			m_pTree.writeNode(this);			
						
			if ((! b || !flag) && ! pathBuffer.empty())
			{
		...
		{
			Node[] nodes = split(pData, mbr, id, doc);
			Node n = nodes[0];
			Node nn = nodes[1];
			...
			if (pathBuffer.empty())
			{
				...
				r.insertEntry(null, (Region) n.m_nodeMBR.clone(), n.m_identifier, n.m_doc);
				r.insertEntry(null, (Region) nn.m_nodeMBR.clone(), nn.m_identifier, nn.m_doc);
	...
	protected void rtreeSplit(byte[] pData, Region mbr, int id, ArrayList group1, ArrayList group2, HashSet doc)
	{
		int cChild;
		...
		HashSet doc1 = (HashSet)m_pDoc[seeds[0]].clone();
		HashSet doc2 = (HashSet)m_pDoc[seeds[1]].clone();
			...
			else
			{
				...
				for (cChild = 0; cChild < m_capacity + 1; cChild++)
				{
					if (mask[cChild] == false)
					{
						...
						d1 = a.getArea() - a1;
						hybrid1 = DIRTree.beta_area*d1 + (1-DIRTree.beta_area)*docdistance(m_pDoc[cChild], doc1);
						Region b = mbr2.combinedRegion(m_pMBR[cChild]);
						d2 = b.getArea() - a2;
						hybrid2 = DIRTree.beta_area*d2 + (1-DIRTree.beta_area)*docdistance(m_pDoc[cChild], doc2);
				...
				if (group == 1)
				{
					...
					doc1.addAll(m_pDoc[sel]);
				}
				else
				{
					Region.combinedRegion(mbr2, m_pMBR[sel]);
					doc2.addAll(m_pDoc[sel]);
				}
			}
		}
	}
	...
	protected void load(byte[] data) throws Exception
	{
		m_nodeMBR = (Region) m_pTree.m_infiniteRegion.clone();
		m_doc = new HashSet();
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			if(this.isLeaf()){
				m_pDoc[cChild] = DIRTree.objstore.readSet(m_pIdentifier[cChild]);
			}
			else{
				m_pDoc[cChild] = (HashSet)DIRTree.docbtree.find(m_pIdentifier[cChild]);
			}
			m_doc.addAll(m_pDoc[cChild]);
		}
	}

	protected byte[] store() throws Exception
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			if(!this.isLeaf())
				DIRTree.docbtree.insert(m_pIdentifier[cChild], m_pDoc[cChild], true);	
		}
	...
	protected double docdistance(HashSet small, HashSet big){
		
		if(big.size() == 0)
			return 1;
		
		HashSet intersection = (HashSet)small.clone();		
